...
class LMOTS_Test final : public Text_Based_Test {
   public:
      LMOTS_Test() : Text_Based_Test("pubkey/lmots.vec", "Seed,I,q,Msg,PublicKey,Sig") {}
      ...
      Test::Result run_one_test(const std::string&, const VarMap& vars) final {
         ...
         const auto sig_ref = Botan::LMOTS_Signature_Bytes(vars.get_req_bin("Sig"));

         auto sig_ref_slicer = Botan::BufferSlicer(sig_ref);
         auto sig_ref_obj = Botan::LMOTS_Signature::from_bytes_or_throw(sig_ref_slicer);
         auto params = Botan::LMOTS_Params::create_or_throw(sig_ref_obj.algorithm_type());
         ...
         sk.sign(sig, msg);
         result.test_is_eq("Signature generation", sig, sig_ref);
