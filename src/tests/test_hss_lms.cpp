...
// Test signature generation using the raw private key bytes
...
// Test signature verification using the raw public key bytes
...
// Test that for too short signatures, private keys, and public keys a DecodeError appears.
class HSS_LMS_Too_Short_Test final : public Test {
      Test::Result test_too_short_signature() {
         Test::Result result("LMS-HSS");
...
// TODO: this currently only calls APIs not covered by other tests.
// Better would be to integrate them probably into a "normal" test.
// Maybe even into the general tests.
