...
Supported_Point_Formats::Supported_Point_Formats(TLS_Data_Reader& reader,
   ...
   for(size_t i = 0; i != len; ++i)
      ...
      if(static_cast<ECPointFormat>(format) == UNCOMPRESSED)
         ...
         includes_uncompressed = std::any_of(std::begin(remaining_formats), std::end(remaining_formats), [](uint8_t format)
            {
            return static_cast<ECPointFormat>(format) == UNCOMPRESSED;
            });
         break;
         }
   ...
   //   but only if the Supported Groups extension was send.
